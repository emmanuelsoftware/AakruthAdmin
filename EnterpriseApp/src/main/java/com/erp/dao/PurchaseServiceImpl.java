...
public class PurchaseServiceImpl implements PurchaseService {
	...
	@Override
	public Purchase findOne(Integer purId) {

		return purchaseRepository.findOne(purId);
	}

	@Override
	public DataTablesOutput<Purchase> findAll(DataTablesInput input) {
	...
	@Override
	public boolean delete(Integer purId,User user) {
		try {
			Purchase purchase = purchaseRepository.findOne(purId);
			purchase.setAudUsrId(user.getUsrId());
			purchaseRepository.save(purchase);
	...
	@Override
	public List<Purchase> findBySta(char sta) {
	...
	public BigDecimal purchaseTotal(Date from, Date to) {
		List<Purchase> purchases = purchaseRepository.findBySta('L');
		BigDecimal amount = new BigDecimal(0);
		for (Purchase purchase : purchases) {
			amount.add(purchase.getProduct().getPurAmt().multiply(new BigDecimal(purchase.getCnt())));
		}
		return amount;
	}
